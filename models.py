...
class Critic(RLNN):
    ...
    def forward(self, x, u):
        ...
        if not self.layer_norm:
            x = nn.LeakyReLU(self.l1(torch.cat([x, u], 1)))
            x = nn.LeakyReLU(self.l2(x))
            x = self.l3(x)

        else:
            x = nn.LeakyReLU(self.n1(self.l1(torch.cat([x, u], 1))))
            x = nn.LeakyReLU(self.n2(self.l2(x)))
            x = self.l3(x)
...
class CriticTD3(RLNN):
    ...
    def forward(self, x, u):
        ...
        if not self.layer_norm:
            x1 = nn.LeakyReLU(self.l1(torch.cat([x, u], 1)))
            x1 = nn.LeakyReLU(self.l2(x1))
            x1 = self.l3(x1)

        else:
            x1 = nn.LeakyReLU(self.n1(self.l1(torch.cat([x, u], 1))))
            x1 = nn.LeakyReLU(self.n2(self.l2(x1)))
            x1 = self.l3(x1)

        if not self.layer_norm:
            x2 = nn.LeakyReLU(self.l4(torch.cat([x, u], 1)))
            x2 = nn.LeakyReLU(self.l5(x2))
            x2 = self.l6(x2)

        else:
            x2 = nn.LeakyReLU(self.n4(self.l4(torch.cat([x, u], 1))))
            x2 = nn.LeakyReLU(self.n5(self.l5(x2)))
            x2 = self.l6(x2)
